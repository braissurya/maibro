...
<body>
	...
	<div class="block" id="block-tables">	
		 <div class="secondary-navigation">
            <ul class="wat-cf">
              <li class="headerTitle" ><a href="#"> Input KPR</a></li>             
            </ul>
          </div>
		 ...
		 <div class="content">
       	...
       	<table class="table">
			...
			<c:forEach items="${listGroupPolicy}" var="p">
			<tr <c:choose><c:when test="${(s.count % 2) eq 0}">class="odd"</c:when><c:otherwise>class="even"</c:otherwise></c:choose>>
				<td></td>
				<td>${p.life_policy_no}</td>
				<td>${p.life_debitur}</td>
				<td>${p.life_produk}</td>
				<td>${p.fire_policy_no}</td>
				<td>${p.fire_debitur}</td>
				<td>${p.fire_produk}</td>
				<td>${p.createuser}</td>
				<td><fmt:formatDate value="${p.createdate}" pattern="dd-MM-yyyy (HH:mm:ss)"/></td>
	
				<td class="last">
					<a href="${path}/kpr/input/edit/${p.id}" title="edit">
	                      <img src="${path }/static/pilu/images/icons/application_edit.png" alt="Edit" />
                    </a>
                    <a href="${path}/kpr/input/view/${p.id}" title="view">
                      <img src="${path }/static/pilu/images/icons/view.png" alt="View" />
                    </a>
                   <a href="javascript:if(confirm('Are you sure want to Transfer \'Id : ${p.id}\' ?'))window.location='${path}/kpr/input/tranfer/${p.id}';" title="transfer">
                ...
                <div class="actions-bar wat-cf">
                  <div class="actions">
                    <button class="button" type="button" onclick="window.location='${path}/kpr/input/new';">
