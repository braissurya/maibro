...
<body>
	...
	<div class="block" id="block-tables">	
		 <div class="secondary-navigation">
            <ul class="wat-cf">
              <li class="headerTitle" ><a href="#"> Master Konfigurasi</a></li>             
            </ul>
          </div>
		 ...
		 <div class="content">
       	...
       	<table class="table">
				...
				<th>
					<c:choose>
                    	<c:when test="${sort eq 'id'}">
	                    	<c:choose>
				              	<c:when test="${sort_type eq 'asc' }"> <a href="javascript:gotoPage('${page}','id','desc','form')" class="up" >ID Konfigurasi</a></c:when>
				              	<c:when test="${sort_type eq 'desc' }"><a href="javascript:gotoPage('${page}','id','asc','form')" class="down" >ID Konfigurasi</a></c:when>
				              	<c:otherwise><a href="javascript:gotoPage('${page}','id','asc','form')"  >ID Konfigurasi</a></c:otherwise>
			              	</c:choose>
			            </c:when>
		              	<c:otherwise>
		              		<a href="javascript:gotoPage('${page}','id','asc','form')"  >ID Konfigurasi</a>
		              	</c:otherwise>
	              	</c:choose>
				</th>
				<th>
					<c:choose>
                    	<c:when test="${sort eq 'jenis'}">
	                    	<c:choose>
				              	<c:when test="${sort_type eq 'asc' }"> <a href="javascript:gotoPage('${page}','jenis','desc','form')" class="up" >Jenis Konfigurasi</a></c:when>
				              	<c:when test="${sort_type eq 'desc' }"><a href="javascript:gotoPage('${page}','jenis','asc','form')" class="down" >Jenis Konfigurasi</a></c:when>
				              	<c:otherwise><a href="javascript:gotoPage('${page}','jenis','asc','form')"  >Jenis Konfigurasi</a></c:otherwise>
			              	</c:choose>
			            </c:when>
		              	<c:otherwise>
		              		<a href="javascript:gotoPage('${page}','jenis','asc','form')"  >Jenis Konfigurasi</a>
		              	</c:otherwise>
	              	</c:choose>
				</th>
				<th>
					<c:choose>
                    	<c:when test="${sort eq 'keterangan'}">
	                    	<c:choose>
				              	<c:when test="${sort_type eq 'asc' }"> <a href="javascript:gotoPage('${page}','keterangan','desc','form')" class="up" >Keterangan</a></c:when>
				              	<c:when test="${sort_type eq 'desc' }"><a href="javascript:gotoPage('${page}','keterangan','asc','form')" class="down" >Keterangan</a></c:when>
				              	<c:otherwise><a href="javascript:gotoPage('${page}','keterangan','asc','form')"  >Keterangan</a></c:otherwise>
			              	</c:choose>
			            </c:when>
		              	<c:otherwise>
		              		<a href="javascript:gotoPage('${page}','keterangan','asc','form')"  >Keterangan</a>
		              	</c:otherwise>
	              	</c:choose>
				</th>
				<th>
			...
			<c:forEach items="${listMstMaster}" var="p">
			<tr <c:choose><c:when test="${(s.count % 2) eq 0}">class="odd"</c:when><c:otherwise>class="even"</c:otherwise></c:choose>>
				...
				<td class="last">
						<a href="${path}/admin/konfigurasi/edit/${p.id}/${p.jenis}" title="edit">
	                      <img src="${path }/static/pilu/images/icons/application_edit.png" alt="Edit" />
	                    </a>
	                    <a href="${path}/admin/konfigurasi/view/${p.id}/${p.jenis}" title="view">
	                      <img src="${path }/static/pilu/images/icons/view.png" alt="View" />
	                    </a>
<!--	                    <a href="javascript:if(confirm('Are you sure want to Delete \'Id : ${p.id}\' ?'))window.location='${path}/admin/konfigurasi/delete/${p.id}/${p.jenis}';" title="delete">-->
...
<!--	                   </a>-->
                ...
                <div class="actions-bar wat-cf">
                  <div class="actions">
                    <button class="button" type="button" onclick="window.location='${path}/admin/konfigurasi/new';">
