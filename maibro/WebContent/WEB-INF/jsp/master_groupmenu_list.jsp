...
<body>
	...
	<div class="block" id="block-tables">	
		 <div class="secondary-navigation">
            <ul class="wat-cf">
              <li class="headerTitle" ><a href="#"> Master Group Menu</a></li>             
            </ul>
          </div>
		 ...
		 <div class="content">
       	...
       	<table class="table">
				...
				<th>
					<c:choose>
                    	<c:when test="${sort eq 'id'}">
	                    	<c:choose>
				              	<c:when test="${sort_type eq 'asc' }"> <a href="javascript:gotoPage('${page}','id','desc','form')" class="up" >ID Group Menu</a></c:when>
				              	<c:when test="${sort_type eq 'desc' }"><a href="javascript:gotoPage('${page}','id','asc','form')" class="down" >ID Group Menu</a></c:when>
				              	<c:otherwise><a href="javascript:gotoPage('${page}','id','asc','form')"  >ID Group Menu</a></c:otherwise>
			              	</c:choose>
			            </c:when>
		              	<c:otherwise>
		              		<a href="javascript:gotoPage('${page}','id','asc','form')"  >ID Group Menu</a>
		              	</c:otherwise>
	              	</c:choose>
				</th>
				<th>
					<c:choose>
                    	<c:when test="${sort eq 'nama'}">
	                    	<c:choose>
				              	<c:when test="${sort_type eq 'asc' }"> <a href="javascript:gotoPage('${page}','nama','desc','form')" class="up" >Nama Group Menu</a></c:when>
				              	<c:when test="${sort_type eq 'desc' }"><a href="javascript:gotoPage('${page}','nama','asc','form')" class="down" >Nama Group Menu</a></c:when>
				              	<c:otherwise><a href="javascript:gotoPage('${page}','nama','asc','form')"  >Nama Group Menu</a></c:otherwise>
			              	</c:choose>
			            </c:when>
		              	<c:otherwise>
		              		<a href="javascript:gotoPage('${page}','nama','asc','form')"  >Nama Group Menu</a>
		              	</c:otherwise>
	              	</c:choose>
				</th>
				<th>
			...
			<c:forEach items="${listGroupMenu}" var="p">
			<tr <c:choose><c:when test="${(s.count % 2) eq 0}">class="odd"</c:when><c:otherwise>class="even"</c:otherwise></c:choose>>
				...
				<td class="last">
						<a href="${path}/admin/user/${p.id}" title="Group User">
	                      <img src="${path }/static/pilu/images/icons/userlist.png" alt="Group User" />
	                    </a>
						<a href="${path}/admin/groupmenu/edit/${p.id}" title="edit">
	                      <img src="${path }/static/pilu/images/icons/application_edit.png" alt="Edit" />
	                    </a>
	                    <a href="${path}/admin/groupmenu/view/${p.id}" title="view">
	                      <img src="${path }/static/pilu/images/icons/view.png" alt="View" />
	                    </a>
	                    <a href="javascript:if(confirm('Are you sure want to Delete \'Id : ${p.id}\' ?'))window.location='${path}/admin/groupmenu/delete/${p.id}';" title="delete">
                ...
                <div class="actions-bar wat-cf">
                  <div class="actions">
                    <button class="button" type="button" onclick="window.location='${path}/admin/groupmenu/new';">
